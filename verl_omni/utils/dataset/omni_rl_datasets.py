    ...
    def _process_multi_modal_info(
        ...
        if videos is not None:
            ...
            for frames, metadata in videos:
                metadata = {**metadata, "frames_indices": list(map(int, metadata["frames_indices"]))}
                # The current vLLM-Omni processor recovers sampled FPS from duration.
                if metadata.get("duration") is None:
                    metadata["duration"] = metadata["total_num_frames"] / metadata["fps"]
                normalized.append((frames, metadata))
            videos = normalized
