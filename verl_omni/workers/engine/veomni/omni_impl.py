...
from .omni_compat import configure_omni_compat
from .omni_inputs import packed_attention_kwargs, prepare_omni_inputs
...
class OmniVeOmniEngine(VeOmniEngineWithLMHead):
    ...
    def _build_model_optimizer(self):
        ...
        module = configure_omni_compat(module)
        for name, frozen in (
    ...
    def _apply_veomni_input_transforms(self, model_inputs, micro_batch):
        ...
        if packed:
            ...
            model_inputs.update(packed_attention_kwargs(micro_batch["input_ids"].offsets(), pad_size))
        else:
            full_ids = model_inputs["input_ids"]
        prepare_omni_inputs(model_inputs, full_ids, self.module.config.thinker_config)
