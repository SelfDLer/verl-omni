...
METRIC_REGEXES = {
    canonical: [
        re.compile(rf"(?<![\w/])['\"]?{re.escape(alias)}['\"]?\s*[:=]\s*({FLOAT_PATTERN})") for alias in aliases
    ]
    ...
    for record in records:
        ...
        if expected and actual:
            if len(expected) == len(actual):
                bad = [(a, b) for a, b in zip(expected, actual, strict=False) if not almost_equal(a, b)]
                mismatch = bool(bad)
            else:
...
def classify(report: dict[str, Any]) -> list[str]:
    ...
    hashes_incomplete = cross["rollout_count"] > 0 and (
    ...
    if actor["token_mismatches"]:
        conclusions.append(
            "Actor 侧 expected/actual video token 数不一致：优先检查 HF processor、grid_thw 和训练输入拼接。"
        )
    if cross["mismatches"]:
    ...
    if actor_seconds and vllm_seconds and actor_seconds != vllm_seconds:
        conclusions.append(
            "Actor 与 vLLM 观察到的 second_per_grid 值集合不同：这是 mRoPE position_ids 不一致的强信号。"
        )
    ...
    if offpolicy["with_first_position"] and offpolicy["first_position_zero_count"] == offpolicy["with_first_position"]:
        conclusions.append(
            "全部已记录异常都从 response 第 0 个 token 开始，优先怀疑 "
            "prompt/prefill 多模态上下文、mRoPE 或首步 logits，"
            "而不是长回复误差累积。"
        )
    if offpolicy["count"] and offpolicy["with_stable_problem_id"] == 0:
        conclusions.append(
            "OFFPOLICY_SAMPLE 未包含稳定 problem_id；"
            "batch_index 跨 step 不代表同一个样本，暂时不能判断是否固定视频复现。"
        )
    pearson = metric_summaries.get("pearson")
    ppl = metric_summaries.get("log_ppl_abs_diff")
    if pearson and pearson["min"] < 0.995:
        conclusions.append(
            f"全局 Pearson 最低为 {pearson['min']:.6g}（step={pearson['min_step']}"
            "）；结合逐样本结果判断是固定问题样本还是全批次偏差。"
        )
    ...
    if not conclusions:
        conclusions.append(
            "当前日志未发现硬不一致；若仍有 Pearson/PPL 异常，请确认五类日志来自同一次、未 resume 的运行。"
        )
    return conclusions
