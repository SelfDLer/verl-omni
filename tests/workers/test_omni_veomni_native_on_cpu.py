...
def native_model():
    ...
    Qwen3OmniThinkerAdapter.configure_veomni_model(model, None)  # Setup must be idempotent.
    return model.train()
...
def test_native_multimodal_backward(native_model, modality):
    ...
    Qwen3OmniThinkerAdapter.prepare_veomni_inputs(mm, ids, native_model.config)
    output = native_model(
        input_ids=ids,
...
def test_real_verl_input_and_logprob_contract(native_model, real_engine_class, packed, monkeypatch):
    ...
    if packed:
        ...
        return
    with torch.no_grad():
        output = native_model(**model_inputs, use_cache=False)
    ...
    # Exercise the inherited engine call, which supplies use_cache explicitly.
    # A direct model(**model_inputs) call alone misses duplicate keyword errors.
    cache_flags = []

    def capture_cache(module, args, kwargs):
        cache_flags.append(kwargs["use_cache"])

    def loss_function(model_output, data, dp_group):
        return -model_output["log_probs"].values().mean(), {}

    monkeypatch.setattr(engine, "get_data_parallel_group", lambda: None)
    parent_module = sys.modules[engine.forward_step.__module__]
    monkeypatch.setattr(parent_module, "get_device_name", lambda: "cpu")
    monkeypatch.setattr(parent_module, "get_device_id", lambda: "cpu")
    handle = native_model.register_forward_pre_hook(capture_cache, with_kwargs=True)
    try:
        with torch.no_grad():
            _, metadata = engine.forward_step(batch, loss_function=None, forward_only=True)
        torch.testing.assert_close(metadata["model_output"]["log_probs"].values(), result["log_probs"].values())
        loss, _ = engine.forward_step(batch, loss_function=loss_function, forward_only=False)
        loss.backward()
        assert torch.isfinite(native_model.thinker.lm_head.weight.grad).all()
