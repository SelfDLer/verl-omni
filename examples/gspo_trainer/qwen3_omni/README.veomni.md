...
The Qwen3-Omni training adapter converts HF audio features to VeOmni's flat frame representation and
...
The engine resolves `model_adapter_cls` through the existing `(architecture, stage)`
registry. It calls `configure_veomni_model` before parallel wrapping and
`prepare_veomni_inputs` before slicing encoder inputs for sequence parallelism.
These hooks preserve VeOmni's native forward and split-module hints; the HF/FSDP
`configure_model` hook remains separate. Packing, parallel slicing, precision,
optimizer and weight export remain engine responsibilities.
...
```bash
pytest -q tests/pipelines/test_qwen3_omni_veomni_adapter_on_cpu.py \
